...
export async function listRepositoryFoldersAction(
  input: string | { repositoryId?: string; workspaceId?: string },
) {
  const supabase = await createActionSupabaseClient();
  await requireAuthenticatedUser(supabase);

  // Backward compatibility for string input (repositoryId)
  const params = typeof input === "string" ? { repositoryId: input } : input;

  return listRepositoryFolders(supabase, params);
}

export async function createRepositoryFolderAction(
...
export async function updateRepositoryFolderAction(
...
export async function deleteRepositoryFolderAction(
...
export async function moveRepositoryFolderAction(
  input: { id: string; repositoryId: string; newParentId: string | null },
) {
  const supabase = await createActionSupabaseClient();
  await requireAuthenticatedUser(supabase);
